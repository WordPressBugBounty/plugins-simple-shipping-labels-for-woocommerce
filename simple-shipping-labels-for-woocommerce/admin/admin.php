...
class dimap_SimpleShippingLabels {
	...
	public $default_settings = array(
		...
		'auto_fit_text'				=> 	'on',	// checked checkbox
		'font_default_size'	=>	5,		// mm
		'font_max_size'		=>	15,		// mm
	...
	public function __construct( $plugin_basename )
    {
		...
		// Add custom column to WooCommerce orders table page and populate the column with print buttons.
		add_filter( 'manage_edit-shop_order_columns', array( $this, 'add_print_buttons_column' ) );											// Legacy orders page.
		add_action( 'manage_shop_order_posts_custom_column', array( $this, 'add_print_buttons_to_legacy_orders_page' ) );					// Legacy orders page.
		add_filter( 'woocommerce_shop_order_list_table_columns', array( $this, 'add_print_buttons_column' ) );								// HPOS orders page.
		add_action( 'woocommerce_shop_order_list_table_custom_column', array( $this, 'add_print_buttons_to_hpos_orders_page' ),  10, 2 );	// HPOS orders page.
	...
	function add_meta_boxes() {
		...
		add_meta_box(
			'woocommerce-order-shipping-label',
			'Shipping Label',
			array( $this, 'render_admin_order_page_meta_box' ),
			$screen,
			'side',
			'high'
		);
	...
	function render_admin_order_page_meta_box( $post_or_order_object ) {
		...
		. 'value="' . $order->get_id() . '">'. __( 'Label' ) .'</button>';
	...
	function register_settings() {
		...
		add_settings_field( 'auto_fit_text', 'Auto-fit text', array( $this, 'setting_auto_fit_text'), self::SETTINGS_PAGE_SLUG, self::LABEL_SETTINGS_SECTION_SLUG );
	...
	function plugin_options_validate( $input ) {
		...
		$sanitized_and_validated_input = array(
			...
			'font_default_size'			=>	absint( $input['font_default_size'] ),
			'font_max_size'				=>	absint( $input['font_max_size'] ),
	...
	function setting_auto_fit_text() {
		$checked_state = '';
		if ( !empty($this->settings['auto_fit_text']) && $this->settings['auto_fit_text'] == 'on' ) {
			$checked_state = ' checked';
		}
		?>
		<label for="auto-fit-text">
		<input id="auto-fit-text" name="<?php echo self::OPTION_NAME; ?>[auto_fit_text]" type="checkbox"<?php echo $checked_state; ?>/>Resize fonts to maximal value, upon generating and editing labels.</label>
		<ul>
			<li>
				<label for="text-font-default-size">Font default size :
				<input id="text-font-default-size" class="small-text" name="<?php echo self::OPTION_NAME; ?>[font_default_size]" type="number" min="0"  value="<?php echo esc_attr( $this->settings['font_default_size'] ); ?>" />
				mm</label>
			</li>
			<li>
				<label for="text-font-max-size">Font max size :
				<input id="text-font-max-size" class="small-text" name="<?php echo self::OPTION_NAME; ?>[font_max_size]" type="number" min="0" value="<?php echo esc_attr( $this->settings['font_max_size'] ); ?>" />
				mm</label>
			</li>
		</ul>
		<p class="description"><b>Tip:</b> set the max size to same value as default and enable auto-fit in case the default size overflows (i.e., decrease font where needed).</p>
		<?php
	}
	 ...
	 */
	function add_print_buttons_to_legacy_orders_page( $column ) {
		global $post;
	...
	function add_print_buttons_to_hpos_orders_page( $column, $post_or_order_object ) {
	...
	function add_admin_pages_scripts_and_styles( $hook ) {
		...
		// Get current screen properties to assess on which page we are, in combination with the hook,
		// since same hook may appear in several pages, so we don't want to enqueue redundnt JS scripts
		// on non relevant admin pages.
		$current_screen = get_current_screen();

		// Check if we're on WooCommerce HPOS orders page ('wc-orders') or legacy WordPress 'shop-order',
		// by checking if CustomOrdersTableController exists.
		...
		$wc_orders_pages_hooks = ['edit.php','post.php','woocommerce_page_wc-orders'];		// Legacy WooCommers orders pages and HPOS page.
		if ( in_array($hook, $wc_orders_pages_hooks) && $current_screen->post_type == 'shop_order' ) {
	...
	function generate_shipping_labels_page_html( $orders ) {
		...
		.detail {
			...
			overflow: hidden;
			resize: none;
			font-size: <?php echo $this->settings['font_default_size'] ?>mm;
		}
		...
		<script>
			...
			function simpleShippingLabelsPageOnLoaded() {
				...
				for (let i = 0; i < details.length; i++) {
					...
					// Add input listeners to text field only if font resize setting is enabled.
					<?php
					if ( !empty($this->settings['auto_fit_text']) && $this->settings['auto_fit_text'] == 'on' ) {
						echo "details[i].addEventListener('input', simpleShippingLabelsFitInput);";
					}
					?>
			...
			function simpleShippingLabelsFitInput() {
				...
				// We use mm for font size, so we can't use parseFloat(window.getComputedStyle(this).fontSize) which returns pixels.
				// Thus we set and use a custom meta-data field once the field is edited, otherwise we assume it's a default font size.
				var cur_font_size = this.dataset.fontSize;
				if (cur_font_size === undefined) {
					cur_font_size = <?php echo $this->settings['font_default_size'] ?>;
				}
				cur_font_size = parseFloat(cur_font_size);
				var font_max_size = <?php echo $this->settings['font_max_size'] ?>;
				
				// Increase font until it overflows either in horizonthaly or vertically.
				while ( (this.innerText.length > 0) 
				&& (this.clientWidth < this.parentElement.clientWidth) 
				&& (this.clientHeight < this.parentElement.clientHeight)
				&& (cur_font_size < font_max_size) ) {
					cur_font_size += 0.1;
					this.style.fontSize = cur_font_size + 'mm';
				}

				// Decrease font until it doesn't overflow neither horizonthaly nor vertically.
				while ( (this.clientWidth > this.parentElement.clientWidth) || (this.clientHeight > this.parentElement.clientHeight) ) {
					cur_font_size -= 0.1;
					this.style.fontSize = cur_font_size + 'mm';
				}

				// Store current text field font size in mm for next edit font fit.
				this.dataset.fontSize = cur_font_size.toFixed(1);
			}
		</script>
		</body>
		<?php
